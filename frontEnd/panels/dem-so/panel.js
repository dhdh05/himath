export function mount(container) {
  ...
  const GAME_CONFIG = {
    ...
    levelId: 56,             // ID level trong DB (giả sử level 1 của đếm số là 56 theo file db.sql)
    passScore: 50            // Điểm tối thiểu để qua màn
  };
  ...
  // helper: get UserID safely từ current user
  function getUserId() {
    try {
    ...
    // Fallback: Nếu chưa đăng nhập, dùng HiMathUserId hoặc null
  ...
  function initGame() {
    ...
    correctCount = 0;
    wrongCount = 0;
    startTime = Date.now(); // Reset giờ chơi
  ...
  function handleAnswerClick(clickedButton) {
    ...
    if (questionNumber < GAME_CONFIG.totalQuestions) {
        autoNextTimeout = setTimeout(nextQuestion, 1500); // Tự chuyển câu sau 1.5s
    } else {
        autoNextTimeout = setTimeout(finishGame, 1500);   // Hết câu thì kết thúc
    }
  }
  ...
  async function finishGame() {
    // 1. Tính toán
    const totalTime = Math.round((Date.now() - startTime) / 1000); // giây
    ...
    try {
        ...
        if(data.success) {
            // Có thể thêm hiệu ứng thông báo "Đã lưu điểm"
        }
    } catch(err) {
...
//   // instrumentation
//   // Gọi hàm startPage để ghi giờ bắt đầu vào DB (bảng study_sessions)
//   try { if (window.HiMathStats) window.HiMathStats.startPage('digits-dem-so'); } catch (e) {}
...
//             chosen: Number(clickedButton.dataset.answer), 
//             correct: isCorrect, 
//             ts: Date.now() 
//         };
//         sessionAttempts.push(attemptData); 
        
//         // --- THÊM ĐOẠN NÀY ĐỂ GỬI NGAY LẬP TỨC ---
//         if (window.HiMathStats) {
//             window.HiMathStats.recordAttempt('digits-dem-so', attemptData);
//         }
//         // ------------------------------------------
//     } catch (e) {}
//     resultMessageElement.classList.add('show');
...
//     // Không cần gửi sessionAttempts nữa vì đã gửi từng câu rồi
//     // Chỉ cần báo kết thúc trang
//     try { if (window.HiMathStats) window.HiMathStats.endPage('digits-dem-so'); } catch (e) {}
