export function mount(container) {
      ...
      <div class="game-container" id="practiceGameContainer">
        ...
        <div class="game-page" id="practice-gamePage">
          <div class="game-header">
            <button class="back-btn" id="practice-backBtn"><i class="fas fa-arrow-left"></i> Thoát</button>
            <div class="header-info">
                <div class="current-mode" id="practice-currentMode"><i class="fas fa-plus-circle"></i> Phép Cộng</div>
                <div class="q-counter" style="margin-left:15px; background:rgba(255,255,255,0.2); padding:5px 10px; border-radius:10px;">
                    Câu <span id="practice-qNum">1</span>/${GAME_CONFIG.totalQuestions}
                </div>
            </div>
        ...
        <div class="result-screen" id="endScreen" style="display: none; text-align: center; padding: 40px; color: #fff;">
            ...
            <div style="margin-top: 30px;">
                <button id="practice-restartBtn" class="next-btn" style="background: #4CAF50;">
  ...
  try { if (window.HiMathStats) window.HiMathStats.startPage('practice-tinh-toan'); } catch (e) {}
  ...
  const qNumElement = container.querySelector('#practice-qNum');
  ...
  const restartBtn = container.querySelector('#practice-restartBtn');

  // ---------- HELPERS ----------
  function getUserId() { return window.HiMathUserId || 1; }

  function getLevelId() {
      if (currentMode === 'addition') return 72;
      if (currentMode === 'subtraction') return 73;
      return 74; // both
  }
  ...
  function generateNewQuestion(){ 
      ...
      if (operationType === 'both'){ operationType = Math.random()>0.5 ? 'addition' : 'subtraction'; } 
  ...
  function generateAdditionQuestion(){ 
      const numberOfTerms = getRandomNumber(2,5); const numbers=[]; let sum=0; 
      for (let i=0;i<numberOfTerms;i++){ 
          if (i===numberOfTerms-1){ 
              const maxNumber = 20 - sum; numbers.push(maxNumber<=0?0:getRandomNumber(1,maxNumber)); sum += numbers[i]; 
          } else { 
              const maxForThis = Math.min(10, 20 - sum - (numberOfTerms - i - 1)); 
              const number = getRandomNumber(1, Math.max(1, maxForThis)); numbers.push(number); sum += number; 
          } 
      } 
      currentAnswer = sum; 
  ...
  function generateSubtractionQuestion(){ 
      const numberOfNumbers = getRandomNumber(2,5); const numbers=[]; 
      let firstNumber = getRandomNumber(5,20); numbers.push(firstNumber); let result = firstNumber; 
      for (let i=1;i<numberOfNumbers;i++){ 
          const maxSubtract = result - 1; 
          if (maxSubtract <= 0){ numbers.push(0); } 
          else { const subtractNumber = getRandomNumber(1, maxSubtract); numbers.push(subtractNumber); result -= subtractNumber; } 
      } 
  ...
  function displayQuestion(){ 
      ...
      const numbers = currentQuestion.numbers; const operator = isAddition?'+':'-'; 
      numbers.forEach((number,index)=>{ 
          const numberElement = document.createElement('div'); numberElement.className='number'; numberElement.textContent = number; mathExpression.appendChild(numberElement); 
          if (index < numbers.length -1){ 
              const operatorElement = document.createElement('div'); operatorElement.className='operator'; operatorElement.textContent = operator; mathExpression.appendChild(operatorElement); 
          } 
      }); 
      const equalElement = document.createElement('div'); equalElement.className='equal'; equalElement.textContent='='; mathExpression.appendChild(equalElement); 
      const questionMarkElement = document.createElement('div'); questionMarkElement.className='question-mark'; questionMarkElement.textContent='?'; mathExpression.appendChild(questionMarkElement); 
  }
  ...
  function generateAnswers(){ 
      ...
      while (answers.length<4){ 
          let wrongAnswer; const offset = getRandomNumber(1,3); const shouldAdd = Math.random()>0.5; 
      ...
      shuffled.forEach(answer=>{ 
          const btn = document.createElement('button'); btn.className='answer-btn'; btn.textContent = answer; btn.dataset.value = answer; 
  ...
  function handleAnswerClick(clickedButton){
    ...
    if (isCorrect) { gameCorrect++; playSoundFile('sound_correct_answer_long.mp3'); } 
    else { gameWrong++; playSoundFile('sound_wrong_answer_long.mp3'); }
  ...
  async function finishGame() {
    ...
    const score = Math.round((gameCorrect / GAME_CONFIG.totalQuestions) * 100);
    ...
    container.querySelector('#endTitle').textContent = isPassed ? "Hoàn Thành Xuất Sắc! 🎉" : "Cố Gắng Lần Sau! 💪";
    ...
    // API Call
    try {
        await fetch('http://localhost:3000/api/games/submit', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                student_id: getUserId(),
                level_id: getLevelId(),
                game_type: 'tinh-toan', // type trong DB
                score: score,
                stars: stars,
                is_passed: isPassed,
  ...
  // --- EVENTS ---
  container.querySelector('#practice-additionOption').addEventListener('click', () => showGamePage('addition'));
  container.querySelector('#practice-subtractionOption').addEventListener('click', () => showGamePage('subtraction'));
  container.querySelector('#practice-bothOption').addEventListener('click', () => showGamePage('both'));
  backBtn.addEventListener('click', showSelectionPage);
  restartBtn.addEventListener('click', showSelectionPage);
  ...
  container._practiceCleanup = () => {
    ...
    try { if (window.HiMathStats) window.HiMathStats.endPage('practice-tinh-toan'); } catch (e) {}
