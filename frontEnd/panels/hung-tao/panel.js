export function mount(container) {
  ...
  // --- 1. START TRACKING: Bắt đầu tính giờ chơi ---
  try { 
  ...
  function spawnApple() { 
      ...
      // Tính tốc độ dựa trên điểm số: mỗi 10 điểm tăng tốc độ
      // 0-10 điểm: tốc độ cơ bản (1.2 + random*0.4) - đã tăng từ 0.8 lên 1.2
      // 10-20 điểm: tăng thêm 0.3
      // 20-30 điểm: tăng thêm 0.3 nữa
      // Cứ tiếp tục như vậy
      const baseSpeed = 1.2 + Math.random() * 0.4; // Tốc độ cơ bản (tăng từ 0.8 lên 1.2)
      const speedLevel = Math.floor(score / 10); // Mỗi 10 điểm = 1 level
      const speedIncrease = speedLevel * 0.3; // Mỗi level tăng 0.3
