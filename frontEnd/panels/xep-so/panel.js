export function mount(container) {
  ...
  const GAME_CONFIG = {
    totalQuestions: 10,
    levelId: 79, // ID trong db.sql cho bài Xếp số (1-20)
    passScore: 50
  };
  ...
  let correctCount = 0; // Số câu hoàn thành
  let wrongCount = 0;   // Số lần kéo sai
  let currentSequence = [];
  let hiddenPositions = [];
  let hiddenNumbers = [];
  let draggableNumbers = [];
  let draggedNumber = null;
  let correctSlotsCount = 0; // Số slot đã điền đúng trong câu hiện tại
  ...
  async function finishGame() {
    ...
    try {
        ...
        console.log("Đã lưu kết quả Xếp Số");
