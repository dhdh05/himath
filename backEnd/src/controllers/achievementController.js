...
async function awardAchievement(student_id, game_type, achievement_type, title, description) {
    try {
        // Kiem tra xem da co achievement nay chua
        ...
        if (existing.length === 0) {
            ...
            return true; // Achievement moi duoc trao
        }
        return false; // Da co roi
    } catch (error) {
        console.error("Loi trao achievement:", error);
        return false;
    }
}

// Check va trao achievements sau khi submit score
...
exports.checkAchievements = async (req) => {
    try {
        ...
        // 1. Check perfect_score (dat 100 diem)
        if (score === 100) {
        ...
        // 2. Check first_play (lan dau choi game)
        ...
        // 3. Check level_master (hoan thanh tat ca level cua game)
        if (level_id) {
        ...
        // 4. Check star_collector (thu thap 50 sao)
        ...
        // Return achievements de gameController co the include vao response
        return { success: true, new_achievements: newAchievements };
    } catch (error) {
        console.error("Loi check achievements:", error);
        res.status(500).json({ success: false, message: 'Lỗi server' });
    }
};

// Lay danh sach achievements cua hoc sinh
exports.getAchievements = async (req, res) => {
    try {
        ...
        console.error("Loi lay achievements:", error);
        res.status(500).json({ success: false, message: 'Lỗi server' });
    }
};

// Check streak achievements (goi tu streak controller)
exports.checkStreakAchievements = async (student_id, streak) => {
    try {
        ...
        console.error("Loi check streak achievements:", error);
    }
};
